...
//Update User basic info
  if(isset($_POST['update'])) {
    $req_fields = array('name','mail','username','level');
    ...
    if(empty($errors)){
       ...
       $status   = remove_junk($db->escape($_POST['status']));
            $sql = "UPDATE users SET name ='{$name}',mail='{$mail}',  username ='{$username}',user_level='{$level}',status='{$status}' WHERE id='{$db->escape($id)}'";
         $result = $db->query($sql);
          if($result && $db->affected_rows() === 1){
            $session->msg('s',"Se actualizó correctamente");
          ...
          <form method="post" action="edit_user.php?id=<?php echo (int)$e_user['id'];?>" class="clearfix">
            <div class="form-group">
             ...
             <div class="form-group">
                  ...
                  <input type="text" class="form-control" name="mail" value="<?php echo remove_junk(ucwords($e_user['mail'])); ?>">
            </div>
            <div class="form-group">
